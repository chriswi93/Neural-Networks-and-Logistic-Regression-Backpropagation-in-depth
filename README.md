...
Forward propagation and backpropagation are repeated several times. For logistic regression convergens to the global minimum is guranteed. However, for a neural network that approximate functions with non linear relationships only convergens to a local minimum is guranteed. <br>
