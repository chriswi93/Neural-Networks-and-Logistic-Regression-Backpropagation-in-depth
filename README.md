...
Forward propagation and backpropagation are repeated several times. Convergens to the global minimum is guranteed for logistic regression and convergens to a local minimum is guranteed for neural networks that approximate functions with non linear relationships. That's all you have to know about gradient descent. There are many extensions that are not discussed in this article. Therefore will discusss stochastic gradient descent in another article and why it makes sense to use it. 
